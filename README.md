...
face-api.js fetches weight manifests + shards from /models/ (HEAD-checked before load): tiny_face_detector_model-*, face_landmark_68_model-*, face_recognition_model-* (7 files).
...
Create users in Auth, then for each create a Firestore doc:
// users/{uid}
...
Add all VITE_* env vars in project settings.
npm run build output dir: dist/. Models are bundled via the build hook.
HTTPS is mandatory in production — camera (getUserMedia) and fullscreen APIs require a secure context.
